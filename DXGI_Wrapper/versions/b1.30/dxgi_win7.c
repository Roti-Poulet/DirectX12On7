 ...
 *
 * Compiled with MinGW64: x86_64-w64-mingw32-gcc -m64 -shared -O2 -std=c11 -o dxgi.dll dxgi_win7.c dxgi_win7.def -ldxgi -ld2d1 -loleaut32 -lgdi32 -luser32 dxgi_win7.res
...
static HRESULT STDMETHODCALLTYPE WA_QueryVideoMemoryInfo(WrapAdapter*w, UINT node, UINT sg, void*p)
{
    ...
    if (!w->pReal3) {
        if (p) {
            ...
            // Valeurs réalistes pour éviter que le jeu ne pense que la mémoire est pleine
            info->Budget                = 8ULL * 1024 * 1024 * 1024;   // 8 Go
            info->CurrentUsage          = 1ULL * 1024 * 1024 * 1024;   // 1 Go utilisé
            info->AvailableForReservation = info->Budget - info->CurrentUsage;
            info->CurrentReservation    = 0;
            DXGILog("WA_QueryVideoMemoryInfo: Budget=%llu, CurrentUsage=%llu",
    ...
    const void *pDescVoid, IUnknown **ppRealSC)
{
     ...
     * one. We don't yet drive this from a real GPU fence (that's a bigger
     * follow-up -- CreateFence/Signal/SetEventOnCompletion are wired up as
     * offsets but never actually used), so for now this is a manual-reset
     * event that starts (and stays) signaled: correct, crash-safe behavior
     * for any caller, just without real GPU-paced backpressure yet. */
