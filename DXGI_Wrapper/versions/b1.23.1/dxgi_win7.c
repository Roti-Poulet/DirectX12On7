...
{
    LOG("IDXGISwapChain2::GetFrameLatencyWaitableObject -> NULL (stub)");
    (void)w;
    return NULL;
}
    ...
    const void *pDescVoid, IUnknown **ppRealSC)
{
    ...
    // Create CommandList
    {
        ...
        PFN_CLClose pfnClose = (PFN_CLClose)VS(it->pCmdList, D3D12_GRAPHICS_COMMAND_LIST_CLOSE_OFF);
...
static HRESULT D3D12Interop_Present(WrapSC *w, UINT SyncInterval, UINT Flags, HRESULT *outHr)
{
    ...
    // Close command list
    PFN_CLClose pfnClose = (PFN_CLClose)VS(it->pCmdList, 0x48);
    if (!pfnClose) {
        DXGILog("D3D12Interop_Present: Close vtable is NULL");
        *outHr = E_FAIL;
        return E_FAIL;
    }
    hr = pfnClose(it->pCmdList);
    if (FAILED(hr)) {
        DXGILog("D3D12Interop_Present: Close failed 0x%08X", (unsigned)hr);
        *outHr = hr;
        return hr;
    }
