...
static UINT  g_vramSpoofMB = 8192; /* VRAMAmountSpoofInMB -- reported VRAM budget
                                    * via IDXGIAdapter3::QueryVideoMemoryInfo.
                                    * See WA_QueryVideoMemoryInfo. Was previously
                                    * hardcoded to 8GB; now configurable since
                                    * real hardware VRAM (e.g. 4GB) can be lower
                                    * than what games expect/assume. */
...
static void DXGILog_Init(void)
{
     ...
     */
    {
        ...
        g_vramSpoofMB = (UINT)GetPrivateProfileIntA("dxgi_win7", "VRAMAmountSpoofInMB", 8192, iniPath);
        /* Clamp to something sane: below 256MB isn't a real GPU config and
         * risks AvailableForReservation underflowing against CurrentUsage
         * below; above 65536 (64GB) is almost certainly a typo, not intent. */
...
static HRESULT STDMETHODCALLTYPE SC_SetFullscreenState(WrapSC*w,BOOL fs,void*pTarget)
{
    ...
    if (w->interop) {
        /* The D3D12on7 interop path is windowed-only -- there is no real
         * flip-model exclusive fullscreen to hand it. That's fine for
         * fs=FALSE (game asking to *be* windowed: we already are, trivially
         * succeed) but returning DXGI_ERROR_UNSUPPORTED unconditionally
         * here (including for fs=FALSE!) was almost certainly what made
         * RE Engine conclude the swapchain was broken and tear the whole
         * thing down + recreate it every frame (confirmed via log: this
         * call, ResizeTarget, and GetContainingOutput all returning
         * DXGI_ERROR_UNSUPPORTED right before every single
         * CreateSwapChainForHwnd retry). For fs=TRUE, fail with the
         * specific, well-known "not available" code games are actually
         * written to expect and fall back gracefully from, instead of the
         * generic "unsupported" one. */
        if (!fs) {
            LOG_LEAVE("IDXGISwapChain::SetFullscreenState[interop] (fs=FALSE, already windowed)", S_OK);
            return S_OK;
        }
        LOG_LEAVE("IDXGISwapChain::SetFullscreenState[interop] (fs=TRUE, not available on D3D12on7)",
...
static HRESULT STDMETHODCALLTYPE WA_QueryVideoMemoryInfo(WrapAdapter*w, UINT node, UINT sg, void*p)
{
    ...
    if (!w->pReal3) {
        if (p) {
            ...
            /* Budget comes from VRAMAmountSpoofInMB (dxgi.ini), not the real
             * hardware VRAM -- see g_vramSpoofMB comment. CurrentUsage is a
             * fixed fraction of the budget (1/8th, same ratio as the old
             * hardcoded 8GB/1GB) rather than a fixed 1GB, so a low spoofed
             * value (e.g. someone sets 512MB) can't make CurrentUsage
             * exceed Budget and underflow AvailableForReservation. */
            info->Budget                = (UINT64)g_vramSpoofMB * 1024ULL * 1024ULL;
            info->CurrentUsage          = info->Budget / 8;
            info->AvailableForReservation = info->Budget - info->CurrentUsage;
            info->CurrentReservation    = 0;
            DXGILog("WA_QueryVideoMemoryInfo: Budget=%llu, CurrentUsage=%llu (spoofed from VRAMAmountSpoofInMB=%u)",
...
{
    DXGILog("WF_CreateSwapChainForHwnd: pDev=%p, hwnd=%p, pD=%p, pFD=%p, pRO=%p",
            pDev, hwnd, pD, pFD, pRO);
    if (!pp) return E_INVALIDARG; *pp = NULL;

    // D3D12 detection
    void *pCQ=NULL;
    HRESULT hrcq = pDev ? pDev->lpVtbl->QueryInterface(pDev,&IID_ID3D12CommandQueue,&pCQ) : E_POINTER;
    DXGILog("WF_CreateSwapChainForHwnd: QI(CommandQueue) = 0x%08X, pCQ=%p", (unsigned)hrcq, pCQ);
    if (SUCCEEDED(hrcq) && pCQ) {
        IUnknown *pRealSC=NULL;
        D3D12Interop *it;
        DXGILog("WF_CreateSwapChainForHwnd: D3D12 detected, creating interop");
        it = D3D12Interop_Create((IUnknown*)pCQ, hwnd, pD, &pRealSC);
        ((IUnknown*)pCQ)->lpVtbl->Release((IUnknown*)pCQ);
        if (!it) {
            DXGILog("WF_CreateSwapChainForHwnd: D3D12Interop_Create failed");
            return E_FAIL;
        }
        WrapSC *wsc=NULL;
        HRESULT hr2 = WrapSC_Create2(NULL, &IID_IDXGISwapChain1, pp, &wsc, it);
        DXGILog("WF_CreateSwapChainForHwnd: WrapSC_Create2 = 0x%08X, wsc=%p", (unsigned)hr2, wsc);
        if (FAILED(hr2) || !wsc) {
            D3D12Interop_Destroy(it);
            return hr2;
        }
    ...
    // Fallback
    if (!w->pRealF2) return DXGI_ERROR_UNSUPPORTED;
    ...
    DXGILog("WF_CreateSwapChainForHwnd: real call = 0x%08X, pSC=%p", (unsigned)hr, pSC);
    if (SUCCEEDED(hr)) hr = WrapSC_Create((IUnknown*)pSC,&IID_IDXGISwapChain1,pp);
    if (pSC) ((IUnknown*)pSC)->lpVtbl->Release((IUnknown*)pSC);
    return hr;
}
