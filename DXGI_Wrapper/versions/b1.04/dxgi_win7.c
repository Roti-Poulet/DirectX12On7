...
HRESULT WINAPI CreateDXGIFactory2(UINT Flags, REFIID riid, void **ppFactory)
{
    ...
    // Detect if ppFactory is writable; if not, use riid as ppFactory
    if (!IsWriteablePtr(ppFactory, sizeof(void*))) {
        ...
        if (IsWriteablePtr((void**)riid, sizeof(void*))) {
            ...
            actual_riid = NULL; // will set later
        }
    }

    // Detect if actual_riid is a valid GUID; if not, use a default
    ...
    // Also check if the GUID is all zeros
    if (actual_riid) {
    ...
    HRESULT hr;
    if (g_pfnFactory1) {
        typedef HRESULT(WINAPI*PFN1)(REFIID, void**);
        PFN1 pfn = (PFN1)PIX_Intercept((FARPROC)g_pfnFactory1,
            "GetRealPtrPtrCreateDXGIFactory1", "CreateDXGIFactory1Generated");
        LOG("  CreateDXGIFactory2: calling CreateDXGIFactory1 with riid=%s", GuidName(actual_riid));
        hr = pfn(actual_riid, (void**)&pRaw);
    } else if (g_pfnFactory) {
        typedef HRESULT(WINAPI*PFN0)(REFIID, void**);
        PFN0 pfn = (PFN0)PIX_Intercept((FARPROC)g_pfnFactory,
            "GetRealPtrPtrCreateDXGIFactory", "CreateDXGIFactoryGenerated");
        LOG("  CreateDXGIFactory2: calling CreateDXGIFactory with riid=%s", GuidName(actual_riid));
        hr = pfn(actual_riid, (void**)&pRaw);
    } else {
        LOG("  no DXGI factory function");
    ...
    if (SUCCEEDED(hr) && pRaw) {
        ...
        pRaw->lpVtbl->Release(pRaw);
        LOG("  CreateDXGIFactory2: WrapFactory_Create returned 0x%08X", (unsigned)hr);
    } else {
