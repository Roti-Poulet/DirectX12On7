...
static UINT  g_ovRefreshCooldownMs = 700;
...
static HRESULT STDMETHODCALLTYPE SC_ResizeBuffers(WrapSC*w,UINT C,UINT W,UINT H,UINT F,UINT Fl)
{
    ...
    if (w->interop) {
        ...
        it->presentIndex = 0;
        LOG("SC_ResizeBuffers[interop]: resized to %ux%u, %u buffers, bufIdxMask=0x%X", W, H, C, it->bufIdxMask);
        return S_OK;
    }
