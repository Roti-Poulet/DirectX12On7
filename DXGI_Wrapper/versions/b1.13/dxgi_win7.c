...
static void DXGILog_Init(void)
{
    ...
    /* Place dxgilog.txt next to dxgi.dll itself, not next to the exe.
     * This avoids write permission issues in Minecraft's install directory. */
    GetModuleHandleExA(
    ...
    _snprintf(path, sizeof(path) - 1, "%sdxgilog.txt", dllPath);
    path[sizeof(path) - 1] = '\0';

    g_hLogFile = CreateFileA(
        path,
        GENERIC_WRITE,
        FILE_SHARE_READ,          /* allow tail -f style readers */
        NULL,
        CREATE_ALWAYS,
        FILE_ATTRIBUTE_NORMAL | FILE_FLAG_WRITE_THROUGH, /* flush on every write */
        NULL);
    /* If we can't open next to the dll, fall back to current directory */
...
static void DXGILog(const char *fmt, ...)
{
    ...
    /* Also keep OutputDebugString for debuggers that aren't detected */
...
static void D3D12_DumpInfoQueue(IUnknown *pD3D12Dev)
{
    ...
    if(FAILED(pD3D12Dev->lpVtbl->QueryInterface(pD3D12Dev,&IID_ID3D12InfoQueue,&pIQ)) || !pIQ) {
        /* Silently skip — we already log this once at device creation time */
        return;
    }
    {
    ...
    /* Clear after dump so we don't show the same messages twice */
...
static HRESULT D3D12Interop_Present(WrapSC *w, UINT SyncInterval, UINT Flags, HRESULT *outHr)
{
    ...
    /* Dump D3D12 validation messages only on the first Present (init phase).
     * After that it's a no-op anyway when InfoQueue is unavailable, but
     * skipping the QI entirely avoids overhead on every frame. */
    if (it->presentIndex == 0) {
        D3D12_DumpInfoQueue((IUnknown*)it->pD3D12Dev);
    }
    ...
    /* Always reset allocator + command list before calling Downlevel::Present.
     * The command list is kept in closed state between presents (it is closed
     * once right after CreateCommandList in D3D12Interop_Create, then closed
     * again by Downlevel::Present itself after each call).  We must reopen it
     * every time — including the very first Present. */
    {
