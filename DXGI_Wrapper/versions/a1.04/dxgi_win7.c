/* Everything here is in French. Use google translate if you don't understand french.
 *
 * dxgi_win7.c — Port de dxgi.dll (Windows 10) pour Windows 7 x64
 *
 * Compilé avec MinGW64 : x86_64-w64-mingw32-gcc -m64 -shared -O2 -std=c11 -o dxgi.dll dxgi_win7.c dxgi_win7.def -ldxgi -ld2d1 -loleaut32
...
static HRESULT WrapSC_Create(IUnknown *pReal, REFIID riid, void **ppOut)
{
    ...
    *ppOut=NULL;
    ...
    SC_Release(w); /* retire la ref de création du wrapper */
    return hr;
}
...
static HRESULT STDMETHODCALLTYPE SC_QI(WrapSC *w, REFIID riid, void **ppv)
{
    ...
    if (GuidEq(riid,&IID_IDXGISwapChain1) && w->pReal1)
        { *ppv=w; SC_AddRef(w); return S_OK; }
    if (GuidEq(riid,&IID_IDXGISwapChain2) && w->pReal2)
        { *ppv=w; SC_AddRef(w); return S_OK; }
    if (GuidEq(riid,&IID_IDXGISwapChain3) && w->pReal3)
        { *ppv=w; SC_AddRef(w); return S_OK; }
    if (GuidEq(riid,&IID_IDXGISwapChain4) && w->pReal4)
        { *ppv=w; SC_AddRef(w); return S_OK; }
    /*
...
static HRESULT STDMETHODCALLTYPE SC_Present(WrapSC*w,UINT SyncInterval,UINT Flags)
{
    ...
    HRESULT hr=((PFN)VS(w->base.pReal,0x40))(w->base.pReal,SyncInterval,Flags);
...
static HRESULT STDMETHODCALLTYPE SC_GetBuffer(WrapSC*w,UINT Buf,REFIID riid,void**pp)
{
    ...
    if(!pp) return E_INVALIDARG; *pp=NULL;
    HRESULT hr=((PFN)VS(w->base.pReal,0x48))(w->base.pReal,Buf,riid,pp);
...
static HRESULT STDMETHODCALLTYPE SC_Present1(WrapSC*w,UINT si,UINT fl,const void*p)
{
    ...
    HRESULT hr=((PFN)VS(w->pReal1,0xb0))(w->pReal1,si,fl,p);
...
static HRESULT STDMETHODCALLTYPE WF_CreateSwapChainForHwnd(WrapFactory*w,IUnknown*pDev,HWND hwnd,const void*pD,const void*pFD,void*pRO,void**pp)
{
    ...
    /* --- DIAGNOSTIC : que supporte réellement pDev (vient de D3D12) ? --- */
    {
        void *pu=NULL,*pidd=NULL,*pidd1=NULL,*pidd2=NULL,*pidd3=NULL,*pidd4=NULL;
        HRESULT hru =pDev?pDev->lpVtbl->QueryInterface(pDev,&IID_IUnknown_,&pu):E_POINTER;
        HRESULT hrd =pDev?pDev->lpVtbl->QueryInterface(pDev,&IID_IDXGIDevice,&pidd):E_POINTER;
        HRESULT hrd1=pDev?pDev->lpVtbl->QueryInterface(pDev,&IID_IDXGIDevice1,&pidd1):E_POINTER;
        HRESULT hrd2=pDev?pDev->lpVtbl->QueryInterface(pDev,&IID_IDXGIDevice2,&pidd2):E_POINTER;
        HRESULT hrd3=pDev?pDev->lpVtbl->QueryInterface(pDev,&IID_IDXGIDevice3,&pidd3):E_POINTER;
        HRESULT hrd4=pDev?pDev->lpVtbl->QueryInterface(pDev,&IID_IDXGIDevice4,&pidd4):E_POINTER;
        LOG("  [diag pDev=%p] QI(IUnknown)=0x%08X | QI(IDXGIDevice)=0x%08X pidd=%p | "
            "QI(IDXGIDevice1)=0x%08X | QI(IDXGIDevice2)=0x%08X | QI(IDXGIDevice3)=0x%08X | QI(IDXGIDevice4)=0x%08X",
            (void*)pDev,(unsigned)hru,(unsigned)hrd,pidd,(unsigned)hrd1,(unsigned)hrd2,(unsigned)hrd3,(unsigned)hrd4);
        if(pu)   ((IUnknown*)pu)->lpVtbl->Release((IUnknown*)pu);
        if(pidd) ((IUnknown*)pidd)->lpVtbl->Release((IUnknown*)pidd);
        if(pidd1)((IUnknown*)pidd1)->lpVtbl->Release((IUnknown*)pidd1);
        if(pidd2)((IUnknown*)pidd2)->lpVtbl->Release((IUnknown*)pidd2);
        if(pidd3)((IUnknown*)pidd3)->lpVtbl->Release((IUnknown*)pidd3);
        if(pidd4)((IUnknown*)pidd4)->lpVtbl->Release((IUnknown*)pidd4);
    }
    /* --- fin diagnostic --- */
