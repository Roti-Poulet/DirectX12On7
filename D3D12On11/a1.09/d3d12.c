...
/* Requis par ID3D12Device::GetCopyableFootprints (utilise par le helper
 * UpdateSubresources() de d3dx12.h pour uploader vertex/index/texture
 * buffers via une copie DEFAULT<-UPLOAD). Layout identique a d3d12.h. */
...
struct D12Resource {
    ...
    BOOL                is_d3d11_mapped; /* le d11res est actuellement Map() en D3D11 ?
                                           * DOIT etre FALSE avant d'utiliser la ressource
                                           * comme source/dest de CopySubresourceRegion. */
...
static D3D12_DESCRIPTOR_HEAP_DESC * WINAPI D12DescHeap_GetDesc(IUnknown *s, D3D12_DESCRIPTOR_HEAP_DESC *pOut) {
...
typedef struct {
    ...
    D3D12_DESCRIPTOR_HEAP_DESC* (WINAPI *GetDesc)(IUnknown*, D3D12_DESCRIPTOR_HEAP_DESC*); /* hidden pointer - >8 bytes, this first */
...
static D3D12_RESOURCE_DESC * WINAPI D12Resource_GetDesc(IUnknown *s, D3D12_RESOURCE_DESC *pOut) {
...
static HRESULT WINAPI D12Resource_Map(IUnknown *s, UINT sub, const D3D12_RANGE *r, void **ppData) {
    ...
    *ppData = NULL;
    ...
    /* Cas reel : ressource D3D11 mappable (UPLOAD ou READBACK). On mappe
     * paresseusement en D3D11 et on cache le pointeur/l'etat de map. Si
     * deja mappe (ex: le moteur garde le CB mappe en permanence et
     * rappelle Map plusieurs fois sans Unmap intermediaire), on retourne
     * simplement le pointeur existant sans re-mapper. */
    if (res->d11res && res->device && res->device->d11ctx &&
        res->heap_props.Type != D3D12_HEAP_TYPE_DEFAULT) {

        if (!res->is_d3d11_mapped) {
            D3D11_MAPPED_SUBRESOURCE mapped = {0};
            D3D11_MAP mapType = (res->heap_props.Type == D3D12_HEAP_TYPE_READBACK)
                                 ? D3D11_MAP_READ : D3D11_MAP_WRITE_DISCARD;
            HRESULT hrMap = res->device->d11ctx->lpVtbl->Map(
                res->device->d11ctx, res->d11res, sub, mapType, 0, &mapped);
            if (FAILED(hrMap)) {
                WARN("D12Resource_Map: D3D11 Map echoue pour id=%u: %08lx", res->id, hrMap);
                return hrMap;
            }
            res->mapped_ptr     = mapped.pData;
            res->is_d3d11_mapped = TRUE;
            snprintf(buf, sizeof(buf), "  D3D11 Map reussi, pData=%p\n", mapped.pData);
            OutputDebugStringA(buf);
        } else {
            snprintf(buf, sizeof(buf), "  Deja mappe (D3D11), retourne %p\n", res->mapped_ptr);
            OutputDebugStringA(buf);
        }
    ...
    // Fallback (ex: ressource sans backing D3D11 direct) : buffer systeme.
    if (res->mapped_ptr) {
        *ppData = res->mapped_ptr;
        return S_OK;
    }
    ...
    res->mapped_ptr = ptr;
    *ppData = ptr;
    snprintf(buf, sizeof(buf), "  Map alloué (fallback) %p taille %zu\n", ptr, size);
    OutputDebugStringA(buf);
    return S_OK;
}
...
static void WINAPI D12Resource_Unmap(IUnknown *s, UINT sub, const D3D12_RANGE *r) {
    ...
    (void)r;
    char buf[256];
    snprintf(buf, sizeof(buf), "D12Resource_Unmap: res=%p, sub=%u, mapped_ptr=%p, is_d3d11_mapped=%d\n",
             res, sub, res->mapped_ptr, res->is_d3d11_mapped);
    OutputDebugStringA(buf);

    /* Crucial : on doit reellement lever le mapping D3D11, sinon toute
     * copie ulterieure (CopySubresourceRegion) utilisant cette ressource
     * comme source/dest est invalide et est ignoree silencieusement par
     * le driver (pas de crash, pas d'erreur visible sans debug layer). */
    if (res->is_d3d11_mapped && res->d11res && res->device && res->device->d11ctx) {
        res->device->d11ctx->lpVtbl->Unmap(res->device->d11ctx, res->d11res, sub);
        res->is_d3d11_mapped = FALSE;
    }
}
...
typedef struct {
    ...
    D3D12_RESOURCE_DESC* (WINAPI *GetDesc)(IUnknown*, D3D12_RESOURCE_DESC*);
...
static D3D12_COMMAND_QUEUE_DESC * WINAPI D12CmdQueue_GetDesc(IUnknown *s, D3D12_COMMAND_QUEUE_DESC *pOut);
...
typedef struct {
    ...
    D3D12_COMMAND_QUEUE_DESC* (WINAPI *GetDesc)(IUnknown*, D3D12_COMMAND_QUEUE_DESC*);
} CQVtbl;
    ...
    REFIID riid, void **ppv)
{
    ...
    char logbuf[512];
    snprintf(logbuf, sizeof(logbuf), "D12Dev_CreateCommittedResource: id=%u, dim=%u, width=%llu, height=%u, fmt=%u, heapType=%d, flags=0x%x\n",
             res->id, desc->Dimension, desc->Width, desc->Height, desc->Format, heapProps->Type, desc->Flags);
    ...
    if (desc->Dimension == D3D12_RESOURCE_DIMENSION_BUFFER) {
        ...
        if (SUCCEEDED(hr)) {
            ...
            OutputDebugStringA(logbuf);
            /* IMPORTANT : on ne mappe PLUS le buffer ici a la creation.
             * Le mapping se fait paresseusement dans D12Resource_Map, et
             * D12Resource_Unmap appelle desormais reellement ID3D11
             * DeviceContext::Unmap. Mapper eagerly ici et ne jamais
             * unmapper cassait CopySubresourceRegion (une ressource D3D11
             * mappee ne peut pas servir de source/dest de copie : le
             * driver ignore l'appel silencieusement, sans crash ni log
             * d'erreur visible hors debug layer). C'etait la cause du
             * cube jamais uploade vers le buffer DEFAULT malgre le fix
             * de GetCopyableFootprints. */
        } else {
        ...
        char bfbuf[128];
        snprintf(bfbuf, sizeof(bfbuf), "  Texture2D BindFlags final=0x%x (RENDER_TARGET=%d)\n",
                 td.BindFlags, (td.BindFlags & D3D11_BIND_RENDER_TARGET) ? 1 : 0);
        OutputDebugStringA(bfbuf);
    ...
    const void *pDesc, D3D12_CPU_DESCRIPTOR_HANDLE DestDescriptor)
{
    ...
    snprintf(buf, sizeof(buf), "  ID3D11Device::CreateRenderTargetView hr=0x%08X, res->d11res=%p, res->d11tex=%p, *pSlot=%p\n",
             (unsigned)hr, (void*)res->d11res, (void*)res->d11tex, (void*)*pSlot);
    OutputDebugStringA(buf);
    if (FAILED(hr)) {
...
/* Taille en bits/pixel pour les formats couramment utilises. Best-effort :
 * retombe sur 32 bpp par defaut plutot que de planter. */
static UINT D12_BitsPerPixelForFormat(DXGI_FORMAT fmt)
{
    switch (fmt) {
...
static inline UINT64 D12_AlignUp64(UINT64 v, UINT64 a) { return (v + (a - 1)) & ~(a - 1); }

/* Implementation reelle de GetCopyableFootprints (l'ancien stub ne
 * remplissait AUCUNE des sorties : pLayouts/pNumRows/pRowSizeInBytes/
 * pTotalBytes restaient non-initialises cote appelant). C'est cette
 * fonction que d3dx12.h::UpdateSubresources() appelle pour calculer
 * offset/pitch/taille avant de faire le memcpy dans le buffer UPLOAD
 * puis le CopyBufferRegion/CopyTextureRegion vers le buffer DEFAULT.
 * Sans elle, aucune donnee (vertex/index/texture) n'est jamais copiee
 * vers la ressource DEFAULT reellement utilisee par le rendu. */
static void WINAPI D12Dev_GetCopyableFootprints(IUnknown *s,
    ...
    UINT64 *pTotalBytes)
{
    (void)s; (void)FirstSubresource;

    if (!pResourceDesc || NumSubresources == 0) {
        if (pTotalBytes) *pTotalBytes = 0;
        return;
    }
    ...
    if (pResourceDesc->Dimension == D3D12_RESOURCE_DIMENSION_BUFFER) {
        ...
        if (pLayouts) {
            pLayouts[0].Offset = offset;
            pLayouts[0].Footprint.Format   = DXGI_FORMAT_UNKNOWN;
            pLayouts[0].Footprint.Width    = (UINT)width;
            pLayouts[0].Footprint.Height   = 1;
            pLayouts[0].Footprint.Depth    = 1;
            pLayouts[0].Footprint.RowPitch = (UINT)D12_AlignUp64(width, 256);
        }
        if (pNumRows)        pNumRows[0] = 1;
        if (pRowSizeInBytes) pRowSizeInBytes[0] = width;
        if (pTotalBytes)     *pTotalBytes = offset + width;
        return;
    }

    if (pResourceDesc->Dimension == D3D12_RESOURCE_DIMENSION_TEXTURE2D) {
        UINT bpp      = D12_BitsPerPixelForFormat(pResourceDesc->Format);
        UINT rowBytes = (UINT)(((UINT64)pResourceDesc->Width * bpp + 7) / 8);
        UINT rowPitch = (UINT)D12_AlignUp64(rowBytes, 256); /* D3D12_TEXTURE_DATA_PITCH_ALIGNMENT */
        UINT height   = pResourceDesc->Height ? pResourceDesc->Height : 1;

        if (pLayouts) {
            pLayouts[0].Offset = D12_AlignUp64(offset, 512); /* D3D12_TEXTURE_DATA_PLACEMENT_ALIGNMENT */
            pLayouts[0].Footprint.Format   = pResourceDesc->Format;
            pLayouts[0].Footprint.Width    = (UINT)pResourceDesc->Width;
            pLayouts[0].Footprint.Height   = height;
            pLayouts[0].Footprint.Depth    = 1;
            pLayouts[0].Footprint.RowPitch = rowPitch;
        }
        if (pNumRows)        pNumRows[0] = height;
        if (pRowSizeInBytes) pRowSizeInBytes[0] = rowBytes;
        if (pTotalBytes)     *pTotalBytes = (pLayouts ? pLayouts[0].Offset : offset) + (UINT64)rowPitch * height;
        return;
    }

    WARN("GetCopyableFootprints: dimension %u non geree completement", pResourceDesc->Dimension);
    if (pLayouts)        memset(pLayouts, 0, sizeof(*pLayouts) * NumSubresources);
    if (pNumRows)        memset(pNumRows, 0, sizeof(*pNumRows) * NumSubresources);
    if (pRowSizeInBytes) memset(pRowSizeInBytes, 0, sizeof(*pRowSizeInBytes) * NumSubresources);
    if (pTotalBytes)     *pTotalBytes = 0;
}
